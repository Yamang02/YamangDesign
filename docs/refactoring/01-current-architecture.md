...
| `src/domain/palettes/types.ts` | 팔레트 레이어 타입 (PaletteDefinition, SemanticMapping) |

### 팔레트 레이어 (E01)
| 파일 | 책임 |
|------|------|
| `src/domain/palettes/index.ts` | createPalette() 함수 |
| `src/domain/palettes/presets/` | 팔레트 프리셋 (default, natural, pop, historical 카테고리) |
| `src/domain/palettes/strategies/` | 배경 전략 (light, colored, dark) |
| `src/domain/palettes/mapping/` | 시맨틱 색상 해석 |
...
| 파일 | 책임 |
|------|------|
| `src/domain/constants/theme-presets.ts` | 테마 프리셋 단일 소스 (SOT) |
| `src/domain/constants/semantic-presets.ts` | 커스텀 시맨틱 프리셋 저장 |
| `src/domain/constants/lab-presets.ts` | Lab UI용 프리셋 |
...
```typescript
// 사용자가 팔레트 선택
setPaletteName('spring-cream-soda-01');
