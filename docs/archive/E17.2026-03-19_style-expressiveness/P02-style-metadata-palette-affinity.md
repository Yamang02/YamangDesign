...
- [ ] `BgStrategy` 값 스펙(의미)이 코드/문서에서 일치하는지 확인
- [ ] 경고 UI가 참조하는 `bgStrategy` 소유처(데이터 소스)가 단일화되어 있는지 확인
- [ ] `preferred` vs `incompatible` 표시 우선순위가 결정되어 있는지 확인
- [ ] 타입 에러 없이 빌드 성공 확인

### 구현 시 고정 질문
- `BgStrategy`는 “팔레트 속성값”인가, 아니면 “팔레트의 색 특성에서 파생”되는 값인가?
- 경고/추천 UI는 “현재 선택된 스타일”과 “현재 선택된 팔레트”의 교차정보를 어떤 레벨(ThemeProvider vs Lab 페이지)에서 만들 것인가?
