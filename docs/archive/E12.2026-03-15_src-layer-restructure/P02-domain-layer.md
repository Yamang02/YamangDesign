...
- [x] `src/domain/` 디렉토리 생성
- [x] `palettes/`, `themes/`, `tokens/` 이동
- [x] `styles/` `.ts` 파일 이동 (`domain/styles/`)
- [x] `constants/` 도메인 파일 5개 이동 (`domain/constants/`)
- [x] `utils/palette.ts` 흡수
- [x] `utils/system-colors.ts` 흡수
- [x] `constants/lab-presets.ts` 분리 후 각 위치로 이동
- [x] TypeScript 컴파일 통과
