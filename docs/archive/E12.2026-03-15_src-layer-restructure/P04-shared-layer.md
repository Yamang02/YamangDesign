...
- [x] `src/shared/` 디렉토리 생성
- [x] `utils/` 순수 함수 파일 4개 이동 (`clsx`, `css`, `css-structured`, `color`)
- [x] `shared/utils/index.ts` export 정리
- [x] `styles/*.css` 파일들 `shared/styles/`로 이동
- [x] `@types/` → `shared/@types/`로 이동
- [x] `assets/` → `shared/assets/`로 이동
- [x] `src/utils/` 디렉토리 삭제
- [x] `src/styles/` 디렉토리 삭제 (비어있음 확인 후)
- [x] TypeScript 컴파일 통과
