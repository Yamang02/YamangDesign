...
- 각 작품 페이지는 완전히 자급자족(self-contained)하며, 공통 art 레이어를 추출하지 않는다.
  두 번째 작품이 생길 때 실제 겹치는 패턴을 보고 그때 추출 여부를 결정한다.
- 기존 DS CSS 변수(`--ds-spacing-*`, `--ds-text-*`)는 레이아웃/타이포 일관성 용도로만 참조한다.
  ThemeProvider·token-set에는 변경을 가하지 않는다.
- 페이지는 두 챕터로 구성: Chapter 1(직접 이미지 참조) + Chapter 2(심상 차용 인터랙티브).
