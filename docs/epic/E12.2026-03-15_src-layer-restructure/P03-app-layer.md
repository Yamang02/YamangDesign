...
- [ ] `src/app/` 디렉토리 생성
- [ ] `components/`, `pages/`, `layouts/`, `hooks/`, `state/`, `content/`, `config/` 이동
- [ ] `constants/*-content.ts` 파일들을 `app/content/`로 통합
- [ ] `app/infra/` 신설 및 storage.ts, export.ts 이동
- [ ] `src/constants/` 디렉토리 정리 (비어있으면 삭제)
- [ ] TypeScript 컴파일 통과
