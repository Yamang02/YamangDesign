...
- [ ] `tsconfig.json` / `vite.config.ts` path alias 업데이트
- [ ] 전체 소스 파일 import 경로 수정 (domain, app, shared 모두)
- [ ] CSS import 경로 수정 (`main.tsx`, `App.tsx` 등)
- [ ] `npm run build` 통과
- [ ] `npx tsc --noEmit` 통과
- [ ] `npm run lint` 통과
- [ ] 브라우저에서 주요 페이지 시각적 확인 (Labs, Build, Context)
